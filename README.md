...
SIL-Cipher v1.6 implements the **Linguistic Key Exchange (LKE)**: each position in a message is assigned a different ancient writing system, derived from the shared secret + per-message nonce via HMAC-SHA256. The nonce ensures identical messages produce different ciphertext every time.
...
| Field | Value |
|-------|-------|
| `x-sil-nonce` | 16 random bytes, hex-encoded (32 chars) |
| Generated by | `encode()` — one nonce per message, shared across chunks |
| Used in | All HMAC derivations: `HMAC-SHA256(secret + nonce, purpose:pos)` |
| Transmitted | In SIL header, A2A `x-sil-nonce` field, group message header |
...
| SIL-Cipher v1.6 | Encode + decode EXACT, per-message nonce --- not audited |
...
| npm SDK (`@silence-protocol/sil`) | Published v1.6.0 |
